...
# it has 2 benifies , table get created and file also stored in the azure storage account as processed_db used the mounted path
#
# lap_times_final.write.mode("overwrite").partitionBy('race_id').format("delta").saveAsTable("processed_db.lap_times")
...
primary_key="target.race_id = source.race_id AND target.driver_id = source.driver_id AND target.lap = source.lap"
